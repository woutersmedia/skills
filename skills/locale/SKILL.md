...
name: locale
description: >
  Optional project-specific skill. Use when the project uses locale-prefixed routes
  (app/[lang]/) and absolute API URLs. Enforces absolute URLs for client-side API calls
  and correct locale route structure. Apply when asked to add routes, API calls, or
  localisation features in a project that uses this routing pattern.
metadata:
  internal: true
---
